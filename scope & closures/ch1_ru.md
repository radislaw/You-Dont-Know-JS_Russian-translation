...
# Глава 1: What is Scope?

Одна из самых фундаментальных парадигм почти всех языков программирования - это возможность хранить значения в переменных, и извлекать или изменять эти значения позже. На самом деле, возможность хранить значения в переменных и извлекать их оттуда это то, что делает программу *великолепной*.

Без такой концепции программа сможет выполнить некоторые задачи, но эти задачи будут очень ограниченны и ужасно не интересны.

Но включение переменных в нашу программу порождает более интересные вопросы, к которым мы сейчас обратимся: где обитают эти *переменные*? Другими словами где они хранятся? И, что еще более важно, как наша программа находит их, когда нуждается в них.

Эти вопросы говорят о необходимости четко определенных наборов правил для хранения переменных в определенной локации, и для поиска этих переменных позднее. Мы называем этот набор правил - *Scope*.

Но, где и как установлены эти правила *Scope*?

## Compiler Theory

Это покажется очевидным, или неожиданным, в зависимости от вашего уровня взамодейсвия с различными языками, но не смотря на тот факт, что JavaScript попадает под общую категорию "динамических" или "интерпретируемых" языков, на самом деле это компилируемый язык. Он *не* компилируется заранее, как многие традиционно-компилируемые языки, равно как и результат компиляции portable among various distributed systems.

Но, тем не менее, движок JavaScript выполняет большинство из тех же самых операций любого традиционного языка-компилятора, правда, более изощренными способами, чем мы можем себе представить.

При традиционном compiled-language процессе, кусок исходного кода - ваша программа, обычно *перед* выполнением проходит три этапа, не совсем точно называемых "компиляция":
...
Путь, который мы выбирем для изучения scope - это думать об этом процессе, как о беседе. Но *кто* будет вести эту беседу?
...
**Примечание:** LHS and RHS meaning "left/right-hand side of an assignment" doesn't necessarily literally mean "left/right side of the `=` assignment operator". There are several other ways that assignments happen, and so it's better to conceptually think about it as: "who's the target of the assignment (LHS)" and "who's the source of the assignment (RHS)".
...
**Примечание:** You might be tempted to conceptualize the function declaration `function foo(a) {...` as a normal variable declaration and assignment, such as `var foo` and `foo = function(a){...`. In so doing, it would be tempting to think of this function declaration as involving an LHS look-up.
...
> ***Engine***: Эй *Scope*, I have an RHS reference for `foo`. Ever heard of it?
...
**Примечание:** See the chapter review for the quiz answers!
